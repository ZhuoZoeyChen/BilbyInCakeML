...
declare a_afs_updated_n_def[simp]
...
where
  "afs_inode_to_vnode i \<equiv> \<lparr> v_ino = i_ino i,
  v_nlink = i_nlink i,
  v_size = i_size i,
  v_mtime = i_mtime i,
  v_ctime = i_ctime i,
...
where
  "afs_inode_from_vnode v \<equiv> \<lparr> i_type = (if v_mode v AND s_IFREG \<noteq> 0 then IReg [] else (if v_mode v AND s_IFDIR \<noteq> 0 then IDir Map.empty else ILnk [])),
  i_ino = v_ino v,
  i_nlink = v_nlink v,
  i_size = v_size v,
  i_mtime = v_mtime v,
...
definition
  afs_init_inode :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  vnode \<Rightarrow>  VfsMode \<Rightarrow> ( afs_state \<times>  vnode,  afs_state \<times>  vnode) R cogent_monad"
where
...
definition
  afs_create :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U8 WordArray \<Rightarrow>  VfsMode \<Rightarrow>  vnode \<Rightarrow> (( afs_state \<times>  vnode \<times>  vnode) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_unlink :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U8 WordArray \<Rightarrow>  vnode \<Rightarrow> (( afs_state \<times>  vnode \<times>  vnode) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_link :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U8 WordArray \<Rightarrow>  vnode \<Rightarrow> (( afs_state \<times>  vnode \<times>  vnode) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_mkdir :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U8 WordArray \<Rightarrow>  VfsMode \<Rightarrow>  vnode \<Rightarrow> (( afs_state \<times>  vnode \<times>  vnode) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_rmdir :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U8 WordArray \<Rightarrow>  vnode \<Rightarrow> (( afs_state \<times>  vnode \<times>  vnode) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_symlink :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U8 WordArray \<Rightarrow>  U8 WordArray \<Rightarrow>  VfsMode \<Rightarrow>  vnode \<Rightarrow> (( afs_state \<times>  vnode \<times>  vnode) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_readpage :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U64 \<Rightarrow>  U8 WordArray \<Rightarrow> (( afs_state \<times>  vnode \<times>  U8 WordArray) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_write_begin :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U64 \<Rightarrow>  U32 \<Rightarrow>  U8 WordArray \<Rightarrow> (( afs_state \<times>  vnode \<times>  U8 WordArray) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_write_end :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U64 \<Rightarrow>  U32 \<Rightarrow>  U8 WordArray \<Rightarrow> (( afs_state \<times>  vnode) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_evict_inode :: " afs_state \<Rightarrow>  vnode \<Rightarrow> ( afs_state \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_follow_link :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U8 WordArray \<Rightarrow> (( afs_state \<times>  vnode \<times>  U8 WordArray) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_rename :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U8 WordArray \<Rightarrow>  vnode \<Rightarrow>  U8 WordArray \<Rightarrow>  vnode option \<Rightarrow> (( afs_state \<times>  vnode \<times>  vnode \<times>  vnode option) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
definition
  afs_move :: " afs_state \<Rightarrow>  vnode \<Rightarrow>  U8 WordArray \<Rightarrow>  vnode \<Rightarrow>  vnode \<Rightarrow>  U8 WordArray \<Rightarrow>  vnode option \<Rightarrow> (( afs_state \<times>  vnode \<times>  vnode \<times>  vnode \<times>  vnode option) \<times> ( unit,  ErrCode) R\<^sub>T) cogent_monad"
where
...
where
  "afs_getattr afs stat vnode \<equiv> return (afs, stat \<lparr> vs_ino := v_ino vnode,
  ...
  vs_blocks := ucast (v_size vnode) div 512 + (if (v_size vnode mod 512 > 0) then 1 else 0) \<rparr>)"
...
definition
  iattr_is_set :: " iattr \<Rightarrow>  U32 \<Rightarrow> bool"
where
...
where
  ...
  vnode' = (if iattr_is_set iattr vfs_ATTR_CTIME then vnode' \<lparr> v_ctime := iattr_ctime iattr \<rparr> else vnode') in do
